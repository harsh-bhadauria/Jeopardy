...
So, I built my own. This is a completely free, client-side, zero-restriction Jeopardy editor and cinematic hosting interface designed to run entirely in your browser. 
...
* **A Host-First Interface:** When it's game time, the host gets a cinematic, full-viewport board layout with a dramatic answer fade-in (controlled by spacebar) and smooth modal transitions.
* **Active HUD Scoring:** Scorecards float securely at the bottom of the viewport using click-through empty space (pointer-events) so they never block cell clicks underneath. You can change player names and edit scores on the fly.
* **Typing Quality of Life:** The clue editor has formatting shortcuts for Bold, Italic, and Inline Code with custom caret-preservation logic—so your typing cursor never jumps or resets when you insert images.
